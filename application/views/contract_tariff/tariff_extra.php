...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">PHỤ PHÍ BẬC THANG</div>
			...
			<div class="ibox-body pt-3 pb-3 bg-f9 border-e">
				...
				<div class="row border-e bg-white readonly-temp pt-3" id="temp_info" style="border-top: none!important;">
					<div class="col-xl-4 col-lg-3 col-md-12 col-sm-12 col-xs-12">
						<div class="row form-group">
							<label class="col-lg-4 col-md-4 col-sm-2 col-xs-4 col-form-label">Tên</label>
							<div class="col-lg-8 col-md-8 col-sm-10 col-xs-8">
								<input id="level_name" class="form-control form-control-sm" placeholder="Tên" type="text">
							</div>
						</div>
						<div class="row form-group">
						...
						<table id="contenttable" class="table table-striped display nowrap" cellspacing="0">
							<thead>
								<tr>
									...
									<th col-name="DayLevel" data-type="numeric">Mức Ngày</th>
									...
									<th col-name="UnitCode" class="editor-cancel" show-target="#unitcodes-modal">ĐVT</th>
	...
	<div class="modal-dialog modal-dialog-mw" role="document" style="min-width: 960px">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="groups-modalLabel">Chọn hành trình tàu</h5>
			</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		//------DECLARE VARIABLES
		var _columns = ["rowguid", "STT", "TRF_CODE", "TRF_STD_DESC", "DayLevel", "IX_CD", "CARGO_TYPE", "JOB_KIND", "CNTR_JOB_TYPE", "DMETHOD_CD", "TRANSIT_CD", "IsLocal", "UnitCode", "CURRENCYID", "AMT_F20", "AMT_F40", "AMT_F45", "AMT_E20", "AMT_E40", "AMT_E45", "AMT_NCNTR", "INCLUDE_VAT", "VAT"],
		...
		var dataTbl = tbl.DataTable({
			...
			columnDefs: [{
				...
				},
				{
					...
					targets: _columns.getIndexs(["DayLevel"])
				},
				{
					className: "text-right",
				...
				{
					className: "show-more",
					targets: _columns.getIndexs(["TRF_CODE", "CARGO_TYPE", "JOB_KIND", "CNTR_JOB_TYPE", "DMETHOD_CD", "UnitCode"])
				},
				{
		...
		tbl.moreButton({
			columns: _columns.getIndexs(["TRF_CODE", "CARGO_TYPE", "JOB_KIND", "CNTR_JOB_TYPE", "DMETHOD_CD", "UnitCode"]),
			onShow: function(cell) {
				...
				$("#apply-unitcode").val(cellIdx + "." + _columns.indexOf("UnitCode"));
			}
		});
		//------SET MORE BUTTON FOR COLUMNS
		...
		function saveData() {
			...
			var data = $('#temp').val().split('_');
			var cusID = $('#cusID').val() ? $('#cusID').val() : payers.filter(p => p.VAT_CD == data[2])[0].CusID;
			var fData = {
				'levelName': $('#level_name').val().trim(),
				'oprID': $('#opr').val().trim(),
				'lane': $("#lane").val().trim(),
				'payer': $('#payer').val().trim(),
				'payerType': cusID == "*" ? "*" : getPayerType(cusID),
				'paymentType': $('#payment-type').val().trim(),
				'applyDate': $('#fromDate').val().trim(),
				'expireDate': $('#toDate').val().trim(),
				'ref_mrk': $('#ref_mrk').val().trim(),
				'levelCode': ['#level_name', '#lane', '#opr', '#payer', '#fromDate', '#toDate', '#payment-type', '#ref_mrk']
					.map(e => $(e).val().trim())
					.join("_")
			};
		...
		function postSave(formData) {
			...
			$.ajax({
				url: "<?= site_url(md5('Contract_Tariff') . '/' . md5('ctTRFExtra')); ?>",
				dataType: 'json',
				data: formData,
				type: 'POST',
		...
		function templateChanged() {
			//LevelName, LANE, OPR, PAYER, APPLY_DATE, EXPIRE_DATE, PAYMENT_TYPE, REF_RMK
			...
			if (data.length > 6) {
				...
				$('#level_name, #lane, #payer, #cusID, #fromDate, #toDate, payment-type, #ref_mrk').val("");
			}
		...
		function compareHeader() {
			var data = $('#temp').val();
			var compareData = ['#level_name', '#lane', '#opr', '#payer', '#fromDate', '#toDate', '#payment-type', '#ref_mrk']
				.map(e => $(e).val().trim())
				.join("_");
