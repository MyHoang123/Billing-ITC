						...
						<div class="my-box p-3">
							<div class="row form-group">
								<div class="col-sm-12 input-group">
									...
									<span class="input-group-addon bg-white btn mobile-hiden text-default" style="padding: 0 .5rem"
												title="bỏ chọn tàu" id="remove-ship-selected">
										<i class="ti-close"></i>
									</span>
                            ...
                            <div class="row form-group">
                            	<div class="col-7 input-group input-group-sm">
                                	<select id="cjMode" class="selectpicker form-control" multiple="" title="-- [Phương án] --">
                                        <option value="NTAU">Nhập tàu</option>
                                        <option value="NGTH">Nhập giao thẳng</option>
                                        <option value="XTAU">Xuất tàu</option>
                                        <option value="XGTH">Xuất giao thẳng</option>
                                    </select>
                                </div>
							...
							<div class="row form-group">
								<div class="col-12 input-group input-group-sm">
                                	<select id="oprID" class="selectpicker form-control" title="" data-live-search="true">
                                		<option value="" >-- [Hãng khai thác] --</option>
                                    </select>
                                </div>
							</div>
							<!-- <div class="row form-group">
...
<script type="text/javascript">
	$(document).ready(function () {
		var _colsPayment = ["STT", "TRF_CODE", "TRF_DESC", "INV_UNIT", "JobMode", "DMETHOD_CD", "CARGO_TYPE"
			...
			_colCont = ["rowguid", "STT", "BILL_CHK", "CntrNo", "OprID", "CLASS_Name", "ISO_SZTP", "Status", "CJMode_CD", "Description"
						, "IsLocal", "JobName", "Transit_Name"],
			tblInv = $("#tbl-inv"),
			_colPayer = ["STT", "CusID", "VAT_CD", "CusName", "Address", "CusType"];

		var _colStatis = ["OprID", "CLASS_Name", "JobName", "20F", "40F", "45F", "20E", "40E", "45E"];
		var _results = [], _selectShipKey = '', _listCalc = [], selected_cont = [];
...
////////define table cont
		...
		var dataTblStatis = tblStatis.DataTable({
			scrollY: '17vh',
			columnDefs: [
				{
		...
		$("#search").on("click", function(){
			if( !$("#shipid").val() ){
				$("#shipid").addClass("error");
				$(".toast").remove();
				toastr["error"]("Chưa chọn tàu!");
				return;
			}
                ...
                buttons: {
                    ok: {
                        ...
                        action: function(){
							...
							$.ajax({
				                url: "<?=site_url(md5('Credit') . '/' . md5('creShipTotal'));?>",
				                dataType: 'json',
				                data: formData,
				                type: 'POST',
		...
		function search_ship_total(){
			...
			$.ajax({
				...
				error: function(err){
					...
					$( "#search" ).button("reset");
					$('.toast').remove();
					toastr['error']("Server Error at [search_ship_total]");
					console.log(err);
				}
			});
		}
