...
- Thống kê Containers tồn bãi ngày $timeNow");
 ...
 public function EirWaitPayment()
    {
        $this->data['title'] = "Lệnh chờ thanh toán";
        $access = $this->user->access('EirWaitPayment');
        if ($access === false) {
            show_404();
            exit;
        }
        ...
        if($action === 'loadData') {
            $condition = $this->input->post('condition') ? $this->input->post('condition') : null;
        }
        else {
        ...
        $this->data['listOprID'] = $this->mdltask->getUniqueOprID();
        $this->data['invDraft'] = [];
        $this->data['eir'] = $this->mdltask->getEirDraft($condition);
            foreach ($this->data['eir'] as &$eirItem) {
                $srvData = $this->mdltask->getSRVOrder($eirItem['EIRNo_Draft']);
                if (!empty($srvData)) {
                    // Thêm cột mới nếu có dữ liệu SRV
                    $eirItem['SRV_ORDER'] = $srvData;
                } else {
                    // Nếu không có, có thể gán mảng rỗng hoặc bỏ qua
                    $eirItem['SRV_ORDER'] = [];
                }
            }
        unset($eirItem);

        $this->data['odrs'] = $this->mdltask->getSrvOrderraft($condition);

        foreach ($this->data['odrs'] as &$odrsItem) {
                $srvOrderData = $this->mdltask->getSRVOrder($odrsItem['SSOderNo']);
                if (!empty($srvOrderData)) {
                    // Thêm cột mới nếu có dữ liệu SRV
                    $odrsItem['SRV_ORDER'] = $srvOrderData;
                } else {
                    // Nếu không có, có thể gán mảng rỗng hoặc bỏ qua
                    $odrsItem['SRV_ORDER'] = [];
                }
        ...
        if(!isset($condition['OprID'])) {
            $this->data['invDraft'] = $this->mdltask->getInvDraft();
            foreach ($this->data['invDraft'] as &$eirItem) {
                    $srvData = $this->mdltask->getInvDTLDraft($eirItem['DRAFT_INV_NO']);
                    if (!empty($srvData)) {
                        // Thêm cột mới nếu có dữ liệu SRV
                        $eirItem['SRV_ORDER'] = $srvData;
                    } else {
        ...
        // print_r($this->data['invDraft']);die();
        if($action === 'loadData') {
            echo json_encode([
                "eir" => $this->data['eir'],
                "odrs" => $this->data['odrs'],
                "invDraft" => $this->data['invDraft'],
            ]);
            exit;
        }
        $this->load->view('header', $this->data);
        $this->load->view('task/eir_wait_payment', $this->data);
        $this->load->view('footer');
    }
    ...
    public function tskServiceOrder(){
        ...
        if ($action == "save_draft") {
            ...
            $EIRNo = $this->mdltask->save_SRV_ODR_INV_DRAFT($data, $outInfo);
            if(isset($EIRNo)) {
                echo json_encode(['EIR_Draft'=>True,'EIR'=>$EIRNo]);
                exit;
        ...
        if($action == "view"){
            $act = $this->input->post('act') ? $this->input->post('act') : '';
             if ($act == "checkAmount") {
                ...
                if(count($data) > 0) {
                    $eir = $data['eir'];
                    $checkAmtBk = $this->mdltask->checkBookingAmountNew($eir[0]["BookingNo"], $data['stackingAmount'], count($eir));
                    echo json_encode($checkAmtBk);
                    exit;
                }
                else {
    ...
    // Save EIR draft
        if ($action == "save_draft") {
            ...
            $EIRNo = $this->mdltask->save_EIR_DRAFT($data, $outInfo);
            if(isset($EIRNo)) {
                echo json_encode(['EIR_Draft'=>True,'EIR'=>$EIRNo]);
                exit;
    ...
    public function tskEmptyPickup()
    {
        ...
        if($action == "save") {
            ...
            $manualInvInfo = json_decode($this->session->userdata("invInfo"), true);
            $checkInvNo = isset($manualInvInfo['serial']) && isset($manualInvInfo['invno']) ? $this->mdltask->checkInvNo($manualInvInfo['serial'], $manualInvInfo['invno']) : false;
  ...
  // Save EIR draft
    ...
    public function tskPre_Advice()
    {
       ...
       // Save EIR draft
        ...
        $this->data["ssInvInfo"] = $ssInvInfo;
        $this->data["isDup"] = isset($ssInvInfo['serial']) && isset($ssInvInfo['invno']) ? $this->mdltask->checkInvNo($ssInvInfo['serial'], $ssInvInfo['invno']) : false;
...
EOT;
        ...
        try {
          ...
          public function paymentSuccess(){
            ...
            $manualInvInfo = json_decode($this->session->userdata("invInfo"), true);
            if (isset($manualInvInfo['serial']) && isset($manualInvInfo['invno'])) {
                $checkInvNo = isset($manualInvInfo['serial']) && isset($manualInvInfo['invno']) ? $this->mdltask->checkInvNo($manualInvInfo['serial'], $manualInvInfo['invno']) : false;
            ...
            $outInfo = array();
            $mailTo = null;
            $this->data['message'] = $this->mdltask->paymentQrSuccess($data, $outInfo,  $mailTo);
            if (isset($data['invInfo'])) {
                $this->data['invInfo'] = $data['invInfo'];
            } else {
                if (isset($data["pubType"])) {
          ...
          public function paymentSuccessSrvOrder(){
               $act = $this->input->post('act') ? $this->input->post('act') : '';
            ...
            if ($act == 'use_manual_Inv') {
                ...
                if (count($useInvData) > 0) {
                    ...
                    $checkInvNo = isset($useInvData['serial']) && isset($useInvData['invno']) ? $this->mdltask->checkInvNo($useInvData['serial'], $useInvData['invno']) : false;
       ...
       public function deleteEirDraft()
        {
            $EIRNo = $this->input->post('EIRNo') ? $this->input->post('EIRNo') : '';
            $InvDraftNo = $this->input->post('InvDraftNo') ? $this->input->post('InvDraftNo') : '';
            $Odrs = $this->input->post('Odrs') ? $this->input->post('Odrs') : '';
            $SRV = $this->input->post('SRV') ? $this->input->post('SRV') : '';
            $Resuilt = $this->mdltask->deleteEirDraft($EIRNo, $SRV, $Odrs, $InvDraftNo);
            if(!$Resuilt) {
