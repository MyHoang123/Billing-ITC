...
EOT;
    ...
    public function load_ip_cntr_details($billno = '', $cntrNo = ''){
        ...
        $this->ceh->where('cd.rowguid NOT IN (SELECT Rowguid_CntrDetails FROM EIR_DRAFT WHERE (bXNVC = 0 OR bXNVC IS NULL) AND CntrNo = cd.CntrNo )');
...
EOT;
    ...
    public function save_EIR_DRAFT( $args, &$outInfo = array() ){
        ...
        foreach ( $eirs as $item ) {
            ...
            //Gán số lệnh đầu tiên vào QR Thanh toán
            if( $EIRNo_Draft == '' ){
                $EIRNo_Draft = $item["EIRNo"];
            }
            $item["EIRNo_Draft"] = $EIRNo_Draft;
            $eirSeq = isset( $checkEIRNo[ $item["CJMode_CD"] ] ) ? ( $eirSeq + 1 ) : 1;
     ...
     public function paymentQrSuccess($args, &$outInfo = array(), &$mailTo)
            ...
            if( count( $invInfo ) > 0 ){
                ...
                foreach ($EirDraft as $item) {
                    ...
                    unset($item['EIR_STATUS'], $item['ID'],$item['ShipYear'], $item['ShipVoy'], $item['publishType'], $item['Type_Eir'], $item['RowguidCntrDetails'], $item['EIRNo_Draft']);
                    // Chuẩn hóa dữ liệu unicode
                    ...
                    if ($this->ceh->error()['code'] != 0) {
                     ...
                     if( $args["pubType"] == 'dft' )
                            ...
                            $this->ceh->where("EIRNo_Draft", $args['EirNo'])->delete("EIR_DRAFT");
                            if( isset( $args['odr'] )){
    ...
    try {
        ...
        $payment = $this->ceh
            ...
            $this->ceh->where('EIRNo_Draft', $EirNo)->delete('EIR_DRAFT');
        if ($SRV) {
