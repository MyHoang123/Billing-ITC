...
<style>
	...
	button[data-id="cjMode"] + div.dropdown-menu ul.dropdown-menu,
	button[data-id="oprID"] + div.dropdown-menu ul.dropdown-menu{
...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-body bg-f9 border-e py-2">
				<div class="row">
						...
						<div class="my-box p-3">
							...
							<div class="row">
								<div class="col-7">
									<div class="input-group">
										<input class="form-control form-control-sm mr-2" id="fromDate" type="text" placeholder="Ngày bắt đầu" readonly>
										<input class="form-control form-control-sm" id="toDate" type="text" placeholder="Ngày kết thúc" readonly>
									</div>
								</div>
								<div class="col-5">
									<div class="row form-group">
										<div class="col-md-12 input-group input-group-sm">
											<select id="dMethod" class="selectpicker" data-width="100%" data-style="btn-default btn-sm">
												...
												<?php if(isset($dmethods) && count($dmethods) > 0){ foreach ($dmethods as $item){ ?>
									...
									<div class="row form-group">
										<div class="col-md-12 input-group input-group-sm">
											<select id="transit" class="selectpicker" data-width="100%" data-style="btn-default btn-sm">
												...
												<?php if(isset($transits) && count($transits) > 0){ foreach ($transits as $item){ ?>
								...
								<div class="col-7">
									<div class="row form-group">
										<div class="col-12 input-group-sm input-group">
											<select id="oprID" class="selectpicker form-control" multiple="">
		                                    </select>
										</div>
									</div>
								</div>
							...
							<div class="row">
								<div class="col-7">
									<div class="row form-group">
										<div class="col-12 input-group-sm input-group">
											<select id="cjMode" class="selectpicker form-control" multiple="">
		                                    </select>
										</div>
									</div>
								</div>
									...
									<div class="form-group">
	                                    <div class="mt-2">
	                                        <label class="checkbox checkbox-inline checkbox-primary">
	                                            <input type="checkbox" checked name="jobType" value="1" data-name="ischkCFS">
	                                            <span class="input-span"></span>
	                                            Đóng
	                                        </label>
	                                        <label class="checkbox checkbox-inline checkbox-primary">
	                                            <input type="checkbox" checked name="jobType" value="2" data-name="ischkCFS">
	                                            <span class="input-span"></span>
	                                            Rút
	                                        </label>
	                                        <label class="checkbox checkbox-inline checkbox-primary">
	                                            <input type="checkbox" checked name="jobType" value="1" data-name="IsYardSRV">
	                                            <span class="input-span"></span>
	                                            DV Thường
	                                        </label>
	                                    </div>
...
<script type="text/javascript">
	$(document).ready(function () {
		var _colCont = ["rowguid", "STT", "BILL_CHK", "SSOderNo", "CntrNo", "OprID", "ISO_SZTP", "CLASS_Name", "Status", "Description", "JobName"
						, "DMethod_CD", "IsLocal", "CusID"],
		...
		var _results = [], _selectShipKey = '', tblInv = $("#tbl-inv"), _listCalc = [], selected_cont = [];
...
//------define table
		...
		var dataTblCont = tblCont.newDataTable({
            ...
            columnDefs: [
				...
				{ 
					className: "text-center", targets: _colCont.getIndexs(["STT", "BILL_CHK"])
				},
			],
			buttons: [
				{
		...
		tblStatis.DataTable({
			...
			columnDefs: [
				{
					className: "text-center", targets: _colStatis.getIndexs(["OprID", "CLASS_Name"])
				},
				{
			...
			order: [[ _colStatis.indexOf('OprID'), 'asc' ]],
			paging: false,
            info: false,
            searching: false,
            buttons: [],
...
//------define selectpicker
		$('#oprID').selectpicker({
			actionsBox: true,
			liveSearch: true,
			liveSearchPlaceholder: "Tìm hãng khai thác",
			size: '100%',
			selectAllText: 'Tất cả',
			deselectAllText: 'Hủy chọn',
			noneSelectedText: '-- [Hãng khai thác] --',
			virtualScroll: true
		});
		...
		$('#cjMode').selectpicker({
			actionsBox: true,
			liveSearch: true,
			liveSearchPlaceholder: "Tìm loại công việc",
			size: '100%',
			selectAllText: 'Tất cả',
			deselectAllText: 'Hủy chọn',
			noneSelectedText: '-- [Loại công việc] --',
			virtualScroll: true
		});
//------define selectpicker
...
//------set from date, to date
		...
		$.timepicker.dateRange(
			...
			toDate,
			{
				// minInterval: (1000*60*60*24), // 1day
				// maxInterval: (1000*60*60*24*7), // 1 week
				dateFormat: 'dd/mm/yy',
                ...
                buttons: {
                    ok: {
                        ...
                        action: function(){
							...
							$.ajax({
				                url: "<?=site_url(md5('Credit') . '/' . md5('creYardServiceTotal'));?>",
				                dataType: 'json',
				                data: formData,
				                type: 'POST',
...
//------USING MANUAL INVOICE
        ...
        $("input[name='jobType']").on("change", function(){
        	var notselectJobTypes = $("input[name='jobType']:not(:checked)").map( function(){ return {key: $(this).attr("data-name"), value: this.value} }).get();

        	$("#cjMode option").show();

        	var deselectVal = [];
        	$.each( notselectJobTypes, function(idx, el){
        		$("#cjMode option[key-name='"+ el['key'] +"'][key-value='"+ el['value'] +"']").hide();
        		deselectVal = $("#cjMode option[key-name='"+ el['key'] +"'][key-value='"+ el['value'] +"']:checked").map( function() { return this.value; } ).get();
        	} );

        	var cjModes = $("#cjMode").val( );
        	if( cjModes ){
        		 $("#cjMode").val( cjModes.filter( p => deselectVal.indexOf(p) == -1 ) );
        	}

        	$("#cjMode").selectpicker("refresh");
...
//------FUNCTIONS
		...
		function search_srv_total(){
			...
			var formData = {
				...
				"args" : {
					...
					cjmode: $("#cjMode").val() ? $("#cjMode").val() : [],
					oprs: $("#oprID").val() ? $("#oprID").val() : [],
					...
					jobTypes: $("input[name='jobType']:checked").map( function(){ return {key: $(this).attr("data-name"), value: this.value} }).get()
				}
			};
				...
				success: function (data) {
					if( data.oprs && data.oprs.length > 0 ){
						...
						$.each( data.oprs, function(){
							innerOprHtml += '<option value="'+ this["CusID"] +'" title="'+ this["CusID"] +'">'+this["CusID"] + " : " + this["CusName"]+'</option>';
						} );
						$("#oprID").html('').append(innerOprHtml).selectpicker('refresh');
					}
		...
		function load_CJMode(){
			...
			$.ajax({
				...
				type: 'POST',
				success: function (data) {
					if( data.yardJobs && data.yardJobs.length > 0 ){
						var innerOprHtml = "";

						$.each( data.yardJobs, function(indx, item){
							var keyName = item.IsYardSRV == "1" ? "IsYardSRV" : "ischkCFS";
							var keyValue = item.IsYardSRV == "1" ? "1" : item.ischkCFS;

							innerOprHtml += '<option value="'+ this["CJMode_CD"] +'" title="'+ this["CJMode_CD"] +'" key-name="'+ keyName +'" key-value="'+ keyValue +'">'
												+this["CJMode_CD"] + " : " + this["CJModeName"]
											+'</option>';
						} );
						$("#cjMode").html('').append(innerOprHtml).selectpicker('refresh');
					}
