...
<div class="row">
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-head">
				<div class="ibox-title">ĐỒNG BỘ HOÁ ĐƠN ĐẾN VLS</div>
			</div>
			<div class="ibox-body pt-3 pb-3 bg-f9 border-e">
				<form id="frmdata_export" method="post" action="<?=site_url(md5('Report') .'/'. md5('export_releaseInv'));?>">
					...
					<div class="row border-e bg-white pb-1">
						...
						<div class="col-xs-3 col-md-3 col-lg-3 col-xl-3 mt-3">
							<div class="row form-group">
								<label class="col-sm-3 col-form-label mb-0">Trạng thái</label>
								<div class="col-sm-9">
									<select id="sts" name="sts" class="selectpicker" data-style="btn-default btn-sm bg-white" data-width="100%">
										<option value="*" selected>* (Tất cả)</option>
										<option value="dg">Đã gởi</option>
										<option value="cg">Chưa gởi</option>
									</select>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	$(document).ready(function () {
		var selected;
		var tableHD = $('#contenttable').DataTable({
			scrollY: '45vh',
			columnDefs: [
				{ type: "num", targets: 0 }
			],
			...
			buttons: {
				...
				dom: {
					button: {
						tag: "button",
						className: "btn btn-gradient-peach btn-fix btn-sm"
					},
					buttonLiner: {
						tag: null
					}
		...
		$('#search').on('click', function () {
			...
			$.ajax({
				url: "<?=site_url(md5('Tools') . '/' . md5('syncInvoice'));?>",
				dataType: 'json',
				data: formData,
				type: 'POST',
				success: function (data) {
					...
					if(data.results.length > 0) {
						for (i = 0; i < data.results.length; i++) {
							rows.push([
								(i+1)
								, (!data.results[i].isPosted || data.results[i].isPosted == 0) ? 'Chưa gởi' : 'Đã gởi'
								, (data.results[i].PAYER.length < 10) ? '<span style="color: red">' + data.results[i].PAYER + '</span>' : data.results[i].PAYER
					...
					$('#contenttable').DataTable( {
						...
						buttons: {
							buttons: [{
								...
								action: function ( e, dt, node, config ) {									
									selected = dt.rows({selected: true}).data().toArray();
									if(selected) {
						...
						columnDefs: [
							...
							{ className: "text-right", targets: [8,9,10], render: $.fn.dataTable.render.number( ',', '.', 0) }
						],
						order: [[ 0, 'asc' ]],
						scroller: {
							displayBuffer: 9,
						...
						footerCallback: function ( row, data, start, end, display ) {
				            ...
				            // Update footer
				            $( api.column( 8 ).footer() ).html( $.formatNumber( amt, { format: "#,###", locale: "us" } ) );
				            $( api.column( 9 ).footer() ).html( $.formatNumber( vat, { format: "#,###", locale: "us" } ) );
				            $( api.column( 10 ).footer() ).html( $.formatNumber( tamt, { format: "#,###", locale: "us" } ) );
				        },
